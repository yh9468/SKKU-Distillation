...
class ComputeLossAuxOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        L1feat = nn.L1Loss(reduction='none')
        
        self.distill_obj_loss = nn.BCEWithLogitsLoss()
        
        # Class label smoothing https://arxiv.org/pdf/1902.04103.pdf eqn 3 cp= 1, cn = 0
        self.cp, self.cn = smooth_BCE(eps=h.get('label_smoothing', 0.0))  # positive, negative BCE targets

        # Focal loss 
        # In "Custom hyp" they use original loss (not focal loss, g = 0)
        g = h['fl_gamma']  # focal loss gamma
        if g > 0:
            BCEcls, BCEobj = FocalLoss(BCEcls, g), FocalLoss(BCEobj, g)
        ...
        self.balance = {3: [4.0, 1.0, 0.4]}.get(det.nl, [4.0, 1.0, 0.25, 0.06, .02])  # P3-P7
        self.ssi = list(det.stride).index(16) if autobalance else 0  # stride 16 index
        self.BCEcls, self.BCEobj, self.gr, self.hyp, self.autobalance, self.L1feat = BCEcls, BCEobj, model.gr, h, autobalance, L1feat
        for k in 'na', 'nc', 'nl', 'anchors', 'stride':
            setattr(self, k, getattr(det, k))

    def __call__(self, p, targets, imgs, t_feats=None, t_p=None, s_feats=None, fg_masks=None, bg_masks=None, distill_off=False):  # predictions, targets, model   
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        
        if t_feats is not None:
            lfg, lbg, distill_obj, distill_cls = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        ...
        for i in range(self.nl):  # layer index, layer predictions
            if t_feats is not None:
                ...
                t_feat_aux = t_feats[i + self.nl]
                s_feat_aux = s_feats[i + self.nl]
                
                fg_mask = fg_masks[i]
                bg_mask = bg_masks[i]
                
                lfg += torch.mean(self.L1feat(s_feat, t_feat) * fg_mask)
                lbg += torch.mean(self.L1feat(s_feat, t_feat) * bg_mask)
                
                lfg += torch.mean(self.L1feat(s_feat_aux, t_feat_aux) * fg_mask)
                lbg += torch.mean(self.L1feat(s_feat_aux, t_feat_aux) * bg_mask)
                
                # logit distillation
                s_obj = p[i][..., 4]
                t_obj = t_p[i][..., 4].sigmoid()
                
                s_cls = p[i][..., 5:]
                t_cls = t_p[i][..., 5:]
                
                distill_obj += self.distill_obj_loss(s_obj, t_obj)
                distill_cls += KLDivergenceLoss(s_cls, t_cls, T=0.1)
                
                #norms = max(1.0, fg_mask.sum() * 2)
                #norms_back = max(1.0, bg_mask.sum() * 2)
                
                #lfg += (torch.pow(s_feat - t_feat, 2) * fg_mask).sum() / norms 
                #lbg += (torch.pow(s_feat - t_feat, 2) * bg_mask).sum() / norms_back
                
                #lfg += (torch.pow(s_feat_aux - t_feat_aux, 2) * fg_mask).sum() / norms
                #lbg += (torch.pow(s_feat_aux - t_feat_aux, 2) * bg_mask).sum() / norms_back
        ...
        if t_feats is not None:
            if distill_off:
                ...
                lfg *= 0.03
                lbg *= 0.0
            loss = lbox + lobj + lcls + lfg + lbg + distill_obj + distill_cls
            return loss * bs, torch.cat((lbox, lobj, lcls, lfg, lbg, distill_obj, distill_cls, loss)).detach()
        else:
            loss = lbox + lobj + lcls
            return loss * bs, torch.cat((lbox, lobj, lcls, loss)).detach()
