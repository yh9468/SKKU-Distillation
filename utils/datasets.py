...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    sampler = torch.utils.data.distributed.DistributedSampler(dataset, shuffle=False) if rank != -1 else None
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if nL:
            ...
            mask_labels = xywh2xyxy(labels[:,1:5])
            mask_labels[:,[1,3]] *= img.shape[0]
            mask_labels[:,[0,2]] *= img.shape[1]
            
        mask = np.zeros([1, img.shape[0], img.shape[1]], dtype=np.uint8)
        if nL:
            mask_labels = mask_labels.astype(np.int32)
            for mask_label in mask_labels:
                mask[:, mask_label[1]:mask_label[3], mask_label[0]:mask_label[2]] = 1
        
        # Convert
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        img = np.ascontiguousarray(img)

        return torch.from_numpy(img), labels_out, self.img_files[index], shapes, torch.from_numpy(mask)
